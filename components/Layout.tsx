    ...
    <div className="flex flex-col min-h-screen max-w-lg mx-auto relative px-4 pb-24 pt-6 text-white isolate">
      ...
      <header className="flex justify-between items-center mb-8 px-2 z-10 relative">
        <div className="flex flex-col">
          ...
          <span className="text-[8px] font-sans font-bold uppercase tracking-widest text-indigo-400 opacity-80 -mt-1">Genesis Golden State</span>
        </div>
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md glass border border-white/20 p-2 z-50 shadow-2xl bg-black/80 backdrop-blur-xl">
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`flex flex-col items-center p-2 rounded-xl transition-all duration-300 ${
                activeView === item.id ? 'bg-white/20 scale-110 text-white' : 'text-white/60 hover:text-white'
              }`}
            >
