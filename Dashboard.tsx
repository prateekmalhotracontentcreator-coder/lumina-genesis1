...
const Dashboard: React.FC<DashboardProps> = ({ setActiveView, profile }) => {
  ...
  return (
    <div className="space-y-10 pb-16 animate-enter">
      {/* 1. WELCOME HEADER - Enhanced for Desktop */}
      <header className="px-2 flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
          ...
          <p className="text-[10px] md:text-[12px] font-black text-amber-500 uppercase tracking-[0.4em] mt-2">{APP_CONFIG.edition}</p>
        </div>
        <div className="hidden md:block text-right">
          <p className="text-[10px] text-white/30 uppercase font-black tracking-widest">Active Spiritual Streaks</p>
          <div className="flex gap-4 mt-2 justify-end">
            <div className="glass px-4 py-2 text-xs font-bold border-amber-500/20">🔥 12 Days</div>
            <div className="glass px-4 py-2 text-xs font-bold border-indigo-500/20">📖 48 Chapters</div>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {/* LARGE FEATURE: VERSE HERO (Spans 8 columns on large) */}
        ...
        {/* SIDEBAR WIDGET: REWARDS (Spans 4 columns on large) */}
        <section 
          onClick={() => setActiveView(AppView.ESTORE)}
          className="lg:col-span-4 glass p-8 bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border border-yellow-500/20 flex flex-col justify-center items-center h-full cursor-pointer hover:bg-yellow-500/20 transition-all group"
        >
        ...
        {/* 3. RESPONSIVE MASONRY GRID FOR REMAINING FEATURES */}
        <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <button 
            ...
            className="glass p-8 flex flex-col gap-6 text-left hover:bg-white/10 transition-all border-l-4 border-blue-500 relative overflow-hidden group"
          >
            ...
            <div className="flex -space-x-3 mt-auto">
              {[1, 2, 3, 4].map(i => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-[#1a1c2c] bg-indigo-500 flex items-center justify-center text-[10px] font-bold">U{i}</div>
              ))}
          ...
          {/* SPIRITUAL PATHS GRID - NESTED */}
          <div className="grid grid-cols-1 gap-6">
            <button 
              onClick={() => setActiveView(AppView.MANIFEST)}
              className="glass p-6 flex items-center gap-6 text-left hover:bg-white/10 transition-all border-l-4 border-indigo-500 group"
            >
            ...
            <button 
              ...
              className="glass p-6 flex items-center gap-6 text-left hover:bg-white/10 transition-all border-l-4 border-purple-500 group"
            >
          ...
          <div className="grid grid-cols-2 gap-4">
             <button 
              ...
              className="glass p-6 bg-gradient-to-br from-amber-600/20 to-amber-900/20 border-amber-500/30 flex flex-col items-center justify-center gap-3 text-center active:scale-[0.98] transition-all group"
            >
            ...
            <button 
              ...
              className="glass p-6 bg-gradient-to-br from-blue-600/20 to-indigo-900/20 border-blue-500/30 flex flex-col items-center justify-center gap-3 text-center active:scale-[0.98] transition-all group"
            >
              <span className="text-3xl group-hover:scale-125 transition-transform">🌈</span>
      ...
      {/* 4. HORIZONTAL SECTIONS (Stay horizontal for better scanability) */}
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
          ...
          <button onClick={() => setActiveView(AppView.OCCASIONAL_PRAYERS)} className="hidden md:flex glass p-6 flex-col items-center gap-3 hover:bg-indigo-500/10 transition-all group col-span-2 md:col-span-1">
            <span className="text-3xl group-hover:rotate-12 transition-transform">🙏</span>
      ...
      {/* 5. QUICK PRAYER ACCESS (Desktop Optimized) */}
      <section className="space-y-4">
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 px-2 flex justify-between items-center">
          <span>Occasions</span>
          <button onClick={() => setActiveView(AppView.OCCASIONAL_PRAYERS)} className="text-blue-400 text-[10px] font-black hover:underline transition-all">VIEW ALL SANCTUARY PRAYERS</button>
        </h3>
        <div className="flex gap-4 overflow-x-auto pb-6 no-scrollbar px-1">
          {[
            ...
            <button 
              ...
              className="flex-shrink-0 w-28 h-28 md:w-32 md:h-32 glass flex flex-col items-center justify-center gap-3 hover:bg-indigo-500/20 hover:border-indigo-500/40 transition-all group"
            >
              <span className="text-3xl md:text-4xl group-hover:scale-110 transition-transform">{occ.icon}</span>
              <span className="text-[10px] font-black text-white uppercase tracking-tighter">{occ.label}</span>
            </button>
          ))}
        </div>
      </section>
