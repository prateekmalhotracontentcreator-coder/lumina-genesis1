...
const Layout: React.FC<LayoutProps> = ({ children, activeView, setActiveView, user }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen relative text-white isolate overflow-x-hidden bg-[#0f1018]">
      ...
      <header className="w-full max-w-7xl mx-auto flex justify-between items-center py-6 px-6 md:px-10 z-10 relative">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block border-r border-white/10 pr-4 mr-1">
            <p className="text-[8px] font-black text-green-500/60 uppercase tracking-tighter">Devotion Points</p>
            <p className="text-sm font-bold leading-none">{user?.points || 0}</p>
          </div>
          <button 
            onClick={() => setActiveView(AppView.SETTINGS)}
            className="w-10 h-10 md:w-12 md:h-12 rounded-full glass border border-green-500/10 flex items-center justify-center overflow-hidden hover:border-green-500/50 transition-all shadow-xl group"
          >
            {user?.photoURL ? (
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-xl glass border border-green-500/20 p-2 z-50 shadow-[0_20px_50px_rgba(0,0,0,0.5)] bg-black/70 backdrop-blur-3xl ring-1 ring-green-500/10 md:rounded-3xl">
        <div className="flex justify-around items-center gap-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`text-xl md:text-2xl mb-1 group-hover:scale-110 transition-transform ${isPortal ? 'text-2xl md:text-3xl' : ''} ${isActive && isManifest ? 'text-glow-green text-green-400' : ''}`}>
                  {item.icon}
                </span>
                ...
                {isActive && (
                  <div className={`absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full shadow-lg ${isPortal ? 'bg-indigo-400 shadow-indigo-500/50' : 'bg-green-50'}`} />
                )}
              </button>
            );
          })}
