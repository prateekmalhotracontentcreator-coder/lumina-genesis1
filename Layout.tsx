      ...
      <header className="w-full max-w-7xl mx-auto flex justify-between items-center py-6 px-6 md:px-10 z-10 relative">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block border-r border-white/10 pr-4 mr-1">
            <p className={`text-[8px] font-black uppercase tracking-tighter opacity-60 ${isManifestTab ? 'text-green-500' : 'text-indigo-500'}`}>Resonance Score</p>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div className={`absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full shadow-lg ${
                    isManifestTab ? 'bg-green-500 shadow-green-500/50' : 'bg-indigo-500 shadow-indigo-500/50'
                  }`} />
                )}
              </button>
            );
